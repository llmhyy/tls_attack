...
"""
    ...
    def __init__(self, problem, population):
        ...
        if str(type(population)) != "<class 'Population.Population'>":
            raise RuntimeError('The type of object should be Population')
        self.name = 'EGA'
        ...
        if population.Encoding == 'P':
            ...
            if population.Encoding == 'BG':
                ...
                self.mutFunc = 'mutbga' # breeder GA中的变异算子 实值变异
            else:
                raise RuntimeError('encoding approach should be ''BG''、''RI''或''P''.')
        self.pc = 0.7 # 重组概率
    ...
    def run(self):
        ...
        while self.terminated(population) == False:
            ...
            # 选择
            offspring = population[ea.selecting(self.selFunc, population.FitnV, NIND-1)] #同样选取与父代数量相等个体-1,因为前面还有一个最佳父代
            # 进行进化操作
