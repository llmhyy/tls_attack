...
_,test_idx = utilsDatagen.split_train_test(byte_offset, SPLIT_RATIO, SEED)
# Initialize the normalization function
norm_fn = utilsDatagen.normalize(2, min_max_feature)
# Initialize the batch generator
data_generator = utilsDatagen.BatchGenerator(mmap_data, byte_offset, test_idx, BATCH_SIZE, SEQUENCE_LEN, norm_fn, 
                                                return_seq_len=True, return_batch_idx=True)
# Load the pcap filename for traffic identification
with open(args.namedir) as f:
...
utilsPLot.plot_distribution(mean_acc_for_all_traffic, overall_mean_acc, args.savedir)
###############################################################
# TEST 2
dim_names = []
with open(args.infodir, 'r') as f:
...
utilsPLot.plot_mse_for_dim(mean_squared_error_for_features, dim_names, args.savedir)
###############################################################
# TEST 3
outlier_count = 10
...
target_traffic_name = os.path.split(args.savedir.strip('/'))[-1]
with open(os.path.join(args.savedir, 'predict_log.txt'),'w') as logfile:
